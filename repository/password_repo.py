...
async def generate_secure_password(request: pwd.PasswordGen, db: SessionDep):
    ...
    await db.commit()
    await db.refresh(generated_password)
    return generated_password

async def created_password(request: pwd.PasswordBase, db: SessionDep):
    secret_key = os.getenv("SECRET_KEY")
    if not secret_key:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Encryption key not configured")  
...
async def get_password_by_token(token: str, db: SessionDep):
    stmt = select(pwd.Password).where(pwd.Password.token_url == token)
